...
public interface RolesClaimsRepository extends JpaRepository<RolesClaimsEntity,Long> {
    @Query(value = "SELECT rc.id AS id, rc.created_at AS created_at, rc.updated_at AS updated_at, rc.role_id AS role_id, rc.claim_id AS claim_id " +
    ...
    @Query(value = "select * from roles_claims where role_id=:roleId and claim_id=:claimId",nativeQuery = true)
    Optional<RolesClaimsEntity> findByRoleIdAndClaimId(@Param("roleId")Long roleId,
                                                       @Param("claimId") Long claimId);
    @Modifying
    @Transactional
    @Query(value = "delete from roles_claims where role_id=:roleId and claim_id=:claimId",nativeQuery = true)
    void deleteByRoleIdAndClaimId(@Param("roleId")Long roleId,
                                  @Param("claimId") Long claimId);

    boolean existsByRoleAndClaim(RoleEntity role, ClaimsEntity claim);
