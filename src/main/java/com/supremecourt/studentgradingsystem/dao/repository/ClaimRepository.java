...
public interface ClaimRepository extends JpaRepository<ClaimsEntity,Long> {
    @Query(value = "SELECT c.name AS claimName " +
            "FROM claims c " +
            "JOIN roles_claims rc ON rc.claim_id = c.id " +
    ...
    Optional<ClaimsEntity> findByName(String name);
}
